...
const branches = [];
function branch(curve, width, start, duration, color, leaf = false) {
  const item = { curve, width, start, duration, color, leaf, points: Array.from({ length: 65 }, (_, i) => point(curve, i / 64)) };
  branches.push(item);
  return item;
}
...
const crown = [
  branch([point(trunk.curve, 1), [348, 345], [196, 390], [239, 216]], 18, 0.44, 0.22, colors[0]),
  branch([point(trunk.curve, 1), [449, 342], [617, 368], [574, 208]], 18, 0.44, 0.23, colors[1]),
  branch([point(trunk.curve, 0.68), [298, 402], [183, 535], [113, 399]], 15, 0.34, 0.23, colors[2]),
  branch([point(trunk.curve, 0.78), [486, 383], [622, 530], [689, 389]], 15, 0.37, 0.23, colors[3]),
];
...
function twig(parent, at, end, bend, width, leaf = true) {
  ...
  return branch([origin, [origin[0] + bend, origin[1] - 30], [end[0] - bend * 0.3, end[1] + 35], end], width, parent.start + parent.duration * at, 0.19, parent.color, leaf);
}
const ends = [
...
for (let i = 0; i < 7; i++) {
  ...
  for (let j = 0; j < 2; j++) {
    ...
    branch([p, [p[0] + 14, p[1] + 22], [p[0] + (j ? 38 : -32), p[1] + 20], [p[0] + (j ? 45 : -38), p[1] + 55]], 3, root.start + at * root.duration, 0.16, '#96b7af');
  }
}
branches.forEach(item => { item.start *= 0.88; item.duration *= 0.88; });
export { branches };

function drawBranch(ctx, item, growth) {
  if (growth <= 0) return;
  ...
  points.forEach((p, i) => {
    ...
    const taper = Math.min(1, (growth - t) * 16 + 0.06);
    const width = item.width * (1 - t * (item.width > 20 ? 0.55 : 0.88)) * Math.min(1, growth * 4) * taper / 2;
  ...
  ctx.beginPath();
  [...sides[0], ...sides[1].reverse()].forEach((p, i) => i ? ctx.lineTo(...p) : ctx.moveTo(...p));
  ctx.closePath(); ctx.fillStyle = item.gradient; ctx.fill();
  ctx.beginPath();
  points.forEach((p, i) => i ? ctx.lineTo(...p) : ctx.moveTo(...p));
  ctx.strokeStyle = item.width > 10 ? '#b7aed2' : '#b3cfc1';
  ctx.lineWidth = item.width > 10 ? 1.5 : 0.55; ctx.stroke();
}
...
export function createTreeRenderer(canvas) {
  ...
  const items = branches.map(item => {
    const gradient = ctx.createLinearGradient(item.curve[0][0] - 16, 0, item.curve[0][0] + 22, 0);
    gradient.addColorStop(0, '#3a6269'); gradient.addColorStop(0.48, '#91b6a6'); gradient.addColorStop(0.68, '#777aab'); gradient.addColorStop(1, item.color);
  ...
  return progress => {
    ...
    if (progress > 0) {
      ...
      for (const side of [-1, 1]) {
        ctx.save(); ctx.translate(400 + side * opening * 12, 760);
        ctx.rotate(side * (0.25 + opening * 0.45)); ctx.scale(1 - opening * 0.55, 1 - opening * 0.35);
    ...
    items.forEach(item => drawBranch(ctx, item, clamp((progress - item.start) / item.duration)));
